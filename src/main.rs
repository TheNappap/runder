...
{
    let position = math::Point::new(2.0,2.0,-2.0);
...
fn default_scene(settings: Arc<Settings>) -> SceneGraph{
    ...
    scene_graph.add_object(Box::new(Rectangle::unit_square(Transformation::new().scale_all(4.0).rotate(RotationAxis::Xaxis, -FRAC_PI_2).translate(Vector::new(0.0,0.0,6.0)), Box::new(Lambertian::new(Color::gray(1.0))) )));
